def Ex5(file):
    ...
    #con ^ e $ gli dico che la stringa deve essere lunga quel tot e poi controllo
    #se è costruita in quel modo
    s=f.readlines()
    for i in s:
        if re.search(auto,i)!=None:
            d['auto']+=1
        elif re.search(moto,i)!=None:
            d['moto']+=1
        elif re.search(ciclomotore1,i)!=None:
            d['ciclomotore1']+=1
        elif re.search(ciclomotore2,i)!=None:
            d['ciclomotore2']+=1
        else:
            d['errata']+=1
    return d   
