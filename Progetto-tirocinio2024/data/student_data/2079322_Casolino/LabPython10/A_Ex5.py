def A_Ex5(s,n):
    ...
    if s=='':
        return d
    primelettere=set()
    s=s.strip().split() #creo una lista con tutte le parole della stringa
    for i in s:
        if len(i)>=n:
            primelettere.add(i[0]) #insieme delle prime lettere delle parole lunghe almeno n
    for j in primelettere:
        conta=0
        for elem in s:
            if j==elem[0] and len(elem)>=n:
                conta+=1
        d[j]=d.get(j,0)+conta
    return d
