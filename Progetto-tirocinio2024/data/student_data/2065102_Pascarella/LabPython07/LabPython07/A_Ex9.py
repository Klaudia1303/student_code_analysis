def A_Ex9(l):
    ...
    for i in range(len(l)):
        ...
        for j in range(1, len(stringa)):
            if stringa.count(carattere)<stringa.count(stringa[j]):
                carattere=stringa[j]
            elif stringa.count(carattere)==stringa.count(stringa[j]):
                if ord(carattere)>ord(stringa[j]):
                    carattere=stringa[j]
        insieme.add(carattere)     
    return(insieme)
