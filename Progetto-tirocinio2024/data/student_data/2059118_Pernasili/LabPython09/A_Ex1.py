def A_Ex1(fileName):
    f = open(fileName,encoding="UTF-8") # apro il file
    s = f.read() # legge tutto il file
    c = 0 # contatore
    for i in range (len(s)): # se l'indice è pari ad un numero della lista
        if s[i].isalpha(): # se l'elemento della lista è alfabetico
            c = c+ 1 # aumenta il contatore
            f.close # chiudo il file
    return c # ritorna il numero
