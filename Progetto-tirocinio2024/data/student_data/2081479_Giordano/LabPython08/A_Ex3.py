def A_Ex3(l):
    finalSet=set()
    for string in l:
        for secondString in l:
            stringPair=[]
            if len(string)==len(secondString) and string!=secondString:
                stringPair.append(string)
                stringPair.append(secondString)
                tupleList=tuple(stringPair)
                finalSet.add(tupleList)
    return finalSet
