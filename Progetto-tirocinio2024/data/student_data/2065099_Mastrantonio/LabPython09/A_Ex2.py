def A_Ex2(fileName):
    ...
    for i in range (1,len(l)):
        ...
        elem = elem.strip()
        pos1 = elem.find(',')
        voto = int (elem[pos1+1]+elem[pos1+2])
        if voto >= 18 :
            matricola = str (elem[0]+elem[1]+elem[2]+elem[3])
            pos2 = elem.rfind(',')
            for k in range (1,len(elem)-pos2):
                    materia = materia+ elem[pos2+k]
            risultato.append((matricola, materia))
    risultato.remove((0,0))
    return(risultato) 
