def Ex5(file):
    import re
    d={'auto':0,'moto':0,'ciclomotore1':0,'ciclomotore2':0,'errata':0} #dizionario per immettere le chiavi e far partire i valori da 0
    ...
    for riga in f:
        if re.match(patternauto,riga):    #re.match
            d['auto']+=1
        elif re.match(patternmoto,riga):
            d['moto']+=1
        elif re.match(patternciclomotore1,riga):
            d['ciclomotore1']+=1
        elif re.match(patternciclomotore2,riga):
            d['ciclomotore2']+=1
        else:
            d['errata']+=1
    return d
