def Ex5(file):
    ...
    diz={}
    import re
    keys=('auto', 'moto', 'ciclomotore1', 'ciclomotore2', 'errata')
    pattern=(r'[A-Z]{2}\d{3}[A-Z]{2}', r'[A-Z]{2}\d{5}',\
             r'[A-Z0-9]{6}', r'[A-Z0-9]{5}', r'.*')
    for riga in f:
        for i in range(5):
            if re.match(pattern[i],riga):
                diz[keys[i]]=diz.get(keys[i],0)+1
                break
    return diz
