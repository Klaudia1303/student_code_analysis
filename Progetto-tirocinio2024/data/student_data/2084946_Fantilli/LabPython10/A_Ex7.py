def A_Ex7(fileName):
    ...
    for riga in testo:
        ...
        if int(riga[2])>int(riga[3]):
            diz[riga[0]]=diz.get(riga[0],0)+3
            diz[riga[1]]=diz.get(riga[1],0)
        elif int(riga[2])<int(riga[3]):
            diz[riga[1]]=diz.get(riga[1],0)+3
            diz[riga[0]]=diz.get(riga[0],0)
        else:
            diz[riga[0]]=diz.get(riga[0],0)+1
            diz[riga[1]]=diz.get(riga[1],0)+1
    return diz
