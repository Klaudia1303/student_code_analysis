def A_Ex7(file):
    ...
    for i in range(1, len(r)):
        ...
        for k in range(0,2):
            punteggio = 0
            squadra = partita2[k].strip()
            for u in range(1, len(r)):
                partita = r[u].split(",")
                if squadra in partita:
                    if (partita[0] == squadra and int(partita[2]) > int(partita[3].strip())) or (partita[1] == squadra and int(partita[3].strip())>int(partita[2])):
                        punteggio = punteggio + 3
                    elif int(partita[2]) == int(partita[3].strip()) :
                        punteggio = punteggio + 1
                dizionario[squadra] = (punteggio)
    return(dizionario)
