def A_Ex1(fileName):
    ...
    f = open(fileName)
    stringa = f.read()
    alfabeto = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyzàèìòùóáéíóúÀÉÈÌÒÙ"
    lung_alf = len(alfabeto)
    somma = 0
    for i in range(lung_alf):
        if alfabeto[i] in stringa:
            somma = somma + stringa.count(alfabeto[i])                   
    return(somma)
###############################################################################
