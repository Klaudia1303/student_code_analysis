def Ex5(file):
    ...
    for riga in f:
        ...
        print(riga)
        print(len(riga))
        regex1=r'[A-Z]{2}\d{3}[A-Z]{2}'
        regex2=r'[A-Z]{2}\d{5}'
        regex3=r'[A-Z0-9]{5}'
        regex4=r'[A-Z0-9]{6}'
        if re.search(regex1,riga) and len(riga)==7:
            d['auto']+=1
        elif re.search(regex2,riga) and len(riga)==7:
            d['moto']+=1
        elif re.search(regex3,riga) and len(riga)==5:
            d['ciclomotore1']+=1
        elif re.search(regex4,riga) and len(riga)==6:
            d['ciclomotore2']+=1
        else:
            d['errata']+=1
    f.close()
