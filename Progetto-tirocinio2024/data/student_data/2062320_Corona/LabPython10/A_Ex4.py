def A_Ex4(fileName):
    ...
    for parole in righe:
        ...
        if voto>=18:
            matricola=int(vect[0])
            voto=int(vect[1])
            materia=vect[2]
            
            if matricola not in dizionario:

                dizionario[matricola]=1

            else:
                somma=dizionario.get(matricola)
                dizionario[matricola]=somma+1
