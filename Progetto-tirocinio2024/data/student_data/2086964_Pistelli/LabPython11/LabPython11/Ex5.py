def Ex5(file):
    ...
    for riga in s:
        riga=riga.strip()
        if re.search(auto,riga) and len(riga)==7:
            d['auto']+=1
        elif re.search(moto,riga) and len(riga)==7:
            d['moto']+=1
        elif re.search(ciclomotore1,riga) and len(riga)==5:
            d['ciclomotore1']+=1
        elif re.search(ciclomotore2,riga) and len(riga)==6:
            d['ciclomotore2']+=1
        else:
            d['errata']+=1
    return d   
