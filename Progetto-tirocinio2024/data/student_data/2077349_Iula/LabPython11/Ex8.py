def Ex8(file):
    ...
    mesi = ["B","DHPS","ACELMRT"]
    interval = [(1,28),(1,30),(1,31)]
    s = f.read().splitlines()
    l = []
    for i in s:
        if re.match(pattern,i):
            aa = re.match(pattern,i).group(1)
            mm = re.match(pattern,i).group(2)
            gg = int(re.match(pattern,i).group(3))
            x = ''
            if(gg>=41 and gg<=71):
                gg -= 40
            c = 0
            for j in range(3):
                if mm in mesi[j]:
                    c = 1
                    if gg in range(interval[j][0],interval[j][1]):
