def Ex5(file):
   ...
   with open(file, 'r', encoding='UTF-8') as f:
       for line in f.readlines():
           ...
           if re.match('^[A-Z]{2}[0-9]{3}[A-Z]{2}$',line,re.MULTILINE) is not None:
               d['auto']= d.get('auto',0)+1
           elif re.match('^[A-Z]{2}[0-9]{5}$',line,re.MULTILINE) is not None:
               d['moto']=d.get('moto',0)+1
           elif re.match('^[A-Z0-9]{5}$',line,re.MULTILINE) is not None:
               d['ciclomotore1']=d.get('ciclomotore1',0)+1
           elif re.match('^[A-Z0-9]{6}$',line,re.MULTILINE) is not None:
               d['ciclomotore2']=d.get('ciclomotore2',0)+1
           else:
