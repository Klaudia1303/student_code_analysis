def A_Ex7(file):
    ...
    for r in fin:
        r=r.strip().split(",")
        if int(r[2])>int(r[3]):
            dizionario[r[0]]=dizionario.get(r[0],0)+3
            dizionario[r[1]]=dizionario.get(r[1],0)+0
        elif int(r[2])<int(r[3]):
            dizionario[r[1]]=dizionario.get(r[1],0)+3
            dizionario[r[0]]=dizionario.get(r[0],0)+0
        else:
            dizionario[r[0]]=dizionario.get(r[0],0)+1
            dizionario[r[1]]=dizionario.get(r[1],0)+1
    return dizionario
###############################################################################
