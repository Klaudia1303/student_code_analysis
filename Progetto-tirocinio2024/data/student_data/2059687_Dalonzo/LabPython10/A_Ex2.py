def A_Ex2(file):
    ...
    fileWords = open(file,'r',encoding = 'UTF-8')
    occDict = {}
    index = 1
    indexR = 1
    for riga in fileWords:
        parole = riga.strip().split(' ')
        for parola in parole:
                if(parola not in occDict):
                    occDict[parola] = {indexR}
                elif(indexR not in occDict[parola]):
                    occDict[parola].add(indexR)
        indexR += 1
    fileWords.close()
    return occDict
###############################################################################
